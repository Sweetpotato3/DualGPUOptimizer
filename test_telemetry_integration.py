...
try:
    from dualgpuopt.telemetry import get_telemetry_service, GPUMetrics, hist
...
def test_telemetry_service_with_history():
    ...
    def metrics_callback(metrics: Dict[int, GPUMetrics]):
        ...
        for gpu_id, gpu_metrics in metrics.items():
            print(f"GPU {gpu_id}: Util={gpu_metrics.utilization}%, "
    ...
    for gpu_id in latest_metrics.keys():
        for metric in ["util", "vram", "temp", "power"]:
            ...
            if len(history) < 3:
                print(f"Error: Expected at least 3 samples for {per_gpu_metric}, got {len(history)}")
                success = False
