...
class TestBackwardCompatibility:
    ...
    @pytest.mark.skipif(not HAS_ENHANCED_EVENTS, reason="Enhanced GPU metrics events not available")
...
class TestPublishingMethods:
    ...
    @pytest.mark.skipif(
        not hasattr(EventBus, "publish_async"), reason="Async publishing not supported"
    )
    ...
    def test_universal_publish_method(self, event_bus, event_classes):
        ...
        if (
            hasattr(event_bus, "publish")
            and isinstance(event_bus.publish.__code__.co_varnames, tuple)
            and len(event_bus.publish.__code__.co_varnames) > 2
        ):
            try:
...
class TestComplexScenarios:
    ...
    def test_event_filtering_chain(self, event_bus):
        ...
        try:
            ...
            if (
                len(urgent_events) == 1
                and len(normal_events) == 1
                and len(low_priority_events) == 1
            ):
