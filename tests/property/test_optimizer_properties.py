...
from dualgpuopt.optimizer import Optimizer, ModelParameters, GPUMemoryInfo
...
class TestOptimizerProperties:
    ...
    @settings(max_examples=100, deadline=None)
    @example(
        model=ModelParameters(name="Test", num_layers=32, hidden_size=4096, num_heads=32, context_length=8192),
        gpus=[
            GPUMemoryInfo(gpu_id=0, name="GPU 0", total_memory=24*1024, available_memory=20*1024, is_primary=True),
            GPUMemoryInfo(gpu_id=1, name="GPU 1", total_memory=8*1024, available_memory=6*1024, is_primary=False)
        ]
    )
