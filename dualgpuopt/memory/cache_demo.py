...
class ModelMemoryCalculator:
    ...
    def find_optimal_batch(self, available_memory: int, sequence_length: int) -> int:
        ...
        logger.info(f"Finding optimal batch for memory={available_memory}, seq_len={sequence_length}")
...
def run_demo():
    ...
    small_model = ModelMemoryCalculator(base_size_mb=7 * 1024, token_size_bytes=2 * 1024)  # 7GB model
    large_model = ModelMemoryCalculator(base_size_mb=13 * 1024, token_size_bytes=4 * 1024)  # 13GB model
