...
class MemoryProfiler:
    ...
    def end_inference(self, token_count: int = 0, context: Optional[Dict[str, Any]] = None) -> bool:
        ...
        for event in reversed(self._event_timeline):
            if (
                event.event_type == MemoryEventType.INFERENCE_START
                and event.context.get("inference_id") == inference_id
            ):
    ...
    def _create_matplotlib_visualization(self, parent_widget, width=600, height=400):
        ...
        self._inference_starts = self._ax.plot([], [], "g^", markersize=8, label="Inference Start")[
            0
        ]
    ...
    def _create_text_visualization(self, parent_widget, width=600, height=400):
        ...
        if self._text_widget is None:
            ...
            self._text_widget.insert(
                "end", "Install matplotlib for graphical visualization\n\n", "warning"
            )
    ...
    def _update_text_visualization(self, event_data: Dict[str, Any]) -> None:
        ...
        if not hasattr(self, "_last_text_update") or current_time - self._last_text_update > 1.0:
            ...
            if self._active_session:
                ...
                self._text_widget.insert(
                    "end",
                    f"Session: {self._active_session.session_id} - Running for {elapsed:.1f}s\n",
                    "normal",
                )
                self._text_widget.insert(
                    "end", f"Events: {len(self._event_timeline)}\n\n", "normal"
                )
                ...
                for gpu_id in range(self._monitor._gpu_count):
                    ...
                    self._text_widget.insert(
                        "end",
                        f"GPU {gpu_id}: {stats.get('used', 0):.1f} MB / {stats.get('total', 0):.1f} MB ({usage_percent:.1f}%)\n",
                        tag,
                    )
                ...
                for event in list(reversed(self._event_timeline))[:10]:
                    ...
                    self._text_widget.insert(
                        "end", f"[{event_time:.1f}s] {event.description}\n", tag
                    )
    ...
    def _update_visualization(self, event_data: Dict[str, Any]) -> None:
        ...
        for gpu_id, line in self._lines.items():
            if gpu_id in self._memory_timeline:
                timestamps = [
                    t - self._active_session.start_time for t, _ in self._memory_timeline[gpu_id]
                ]
                memory_values = [
                    m / (1024 * 1024) for _, m in self._memory_timeline[gpu_id]
                ]  # Convert to MB
    ...
    def get_session_report(self, session_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        report = {
            ...
            "duration": (session.end_time if session.end_time else time.time())
            - session.start_time,
            "inference_count": session.inference_count,
            "token_count": session.token_count,
            "events": {event_type.name: 0 for event_type in MemoryEventType},
            "memory_metrics": {},
            "potential_leaks": [],
            "recommendations": [],
        }
    ...
    def export_timeline_data(self, filepath: str, session_id: Optional[str] = None) -> bool:
        ...
        try:
            with open(filepath, "w", newline="") as csvfile:
                ...
                for gpu_id in range(self._monitor._gpu_count):
                    header.extend(
                        [f"gpu{gpu_id}_used_mb", f"gpu{gpu_id}_free_mb", f"gpu{gpu_id}_percent"]
                    )
    ...
    def _take_snapshot(self) -> None:
        ...
        for gpu_id, stats in memory_stats.items():
            ...
            if delta_mb > 1:
                event_type = (
                    MemoryEventType.ALLOCATION if delta > 0 else MemoryEventType.DEALLOCATION
                )
                self._add_event(
                    event_type,
                    gpu_id,
        ...
        for gpu_id, window in self._memory_windows.items():
            ...
            if slope > 0:
                ...
                # Check for leak (steady growth over time)
                    ...
                    and slope_mb_s > self._leak_threshold
                    and window[-1][1] - window[0][1] > 5 * 1024 * 1024
                ):  # > 5 MB change
                    ...
                    for event in reversed(self._event_timeline[-10:]):
                        if (
                            event.event_type == MemoryEventType.LEAK_DETECTED
                            and event.gpu_id == gpu_id
