...
def init_theme(style: ttk.Style):
    ...
    style.map(
        "TNotebook.Tab", background=[("selected", "#371B59")], foreground=[("selected", "white")]
    )
...
class GradientBar(ttk.Canvas):
    def __init__(self, master, w=260, h=14, **kw):
        ...
        self._w, self._h, self._val, self._tar = w, h, 0.0, 0.0
    ...
    def set(self, pct: float):
        self._tar = max(0.0, min(pct, 100.0))

    def _tick(self):
        now = time.perf_counter()
        step = (now - self._last) * 120
        self._last = now
        if abs(self._val - self._tar) > 0.3:
