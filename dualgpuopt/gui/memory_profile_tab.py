...
class MemoryProfileTab(ttk.Frame):
    ...
    def _handle_event(self, event_data: Dict[str, Any]):
        ...
        self.event_text.insert("end", time_str + event_data.get("description", "Unknown event") + "\n", tag)
        self.event_text.see("end")  # Scroll to see latest event

        # Update status line with event count
        self.status_var.set(f"Events: {self._event_count} | Active session: {self._active_session or 'None'}")
    ...
    def _show_session_report(self, session_id):
        ...
        ttk.Label(frame, text=str(report.get("inference_count", 0))).grid(row=2, column=1, sticky="w")
        ...
        for gpu_id, metrics in report.get("memory_metrics", {}).items():
            ...
            ttk.Label(frame, text=f"Min: {min_mb:.1f} MB | Max: {max_mb:.1f} MB | Avg: {avg_mb:.1f} MB").grid(
        ...
        if leaks:
            for i, leak in enumerate(leaks):
                ...
                ttk.Label(frame, text=f"{leak.get('size_mb', 0):.1f} MB at {leak.get('timestamp', 0):.1f}s").grid(
                    row=row, column=1, sticky="w"
                )
                row += 1
        else:
