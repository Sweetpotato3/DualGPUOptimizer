...
class TtkWindow:
    ...
    def __init__(
        self, title="", themename="", size=(800, 600), resizable=(True, True), alpha=1.0, **kwargs
    ):
...
try:
    from dualgpuopt.gpu_info import GPU, get_gpu_info
    ...
    def get_gpu_info() -> List[GPU]:
        return [
            GPU({"name": "Mock GPU 1", "mem_total": 8192}),
...
class ModernApp(WindowClass):
    ...
    def show_chat(self):
        ...
        if not CHAT_AVAILABLE:
            messagebox.showinfo(
                "Feature Unavailable", "Chat feature is not available in this build."
            )
            return
