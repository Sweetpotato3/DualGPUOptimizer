...
class ModelValidator:
    ...
    def validate_context_size(self, ctx_size: int, gpus: list[GPU]) -> tuple[bool, Optional[str]]:
        ...
        if memory_required_gb > total_memory_gb:
            return (
                False,
                f"Context size {ctx_size} requires ~{memory_required_gb:.1f}GB, but only {total_memory_gb:.1f}GB is available",
            )
    ...
    def validate_launch_parameters(
        ...
        if framework == "llama.cpp":
            ...
            if tp_size > len(gpus):
                return (
                    False,
                    f"Tensor parallel size {tp_size} exceeds available GPU count {len(gpus)}",
                )
