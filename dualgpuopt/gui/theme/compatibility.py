...
def apply_theme_with_compatibility(window: Any, theme_data: Dict[str, Any]) -> None:
    ...
    if TTKBOOTSTRAP_AVAILABLE and hasattr(window, 'style') and hasattr(window.style, 'theme_use'):
        try:
            ...
            logger.debug(f"Applying ttkbootstrap theme: {theme_data.get('bootstrap_theme', 'darkly')}")
    ...
    try:
        ...
        style.configure('TLabel', background=theme_data.get('bg', '#1c1c1c'),
                        foreground=theme_data.get('fg', '#ffffff'))
        style.configure('TButton', background=theme_data.get('accent', '#5c5c5c'),
                        foreground=theme_data.get('fg', '#ffffff'))
