...
from dualgpuopt.gui.theme.dpi import DEFAULT_FONT_SIZE, DEFAULT_HEADING_SIZE, scale_font_size
...
def apply_custom_styling(root):
    ...
    style.map(
        "TButton",
        background=[("active", current_theme["accent_light"]), ("disabled", current_theme["bg"])],
    ...
    style.map(
        "ThemeToggle.TButton",
        background=[("active", current_theme["accent"]), ("disabled", current_theme["bg"])],
        foreground=[("disabled", "#AAAAAA")],
    )
    ...
    root.option_add(
        "*TCombobox*Listbox*Font", ("TkDefaultFont", scale_font_size(DEFAULT_FONT_SIZE))
    )
    ...
    style.configure("TPanedwindow", background=current_theme["bg"], sashrelief="flat")
...
def adjust_equilux_colors(root):
    ...
    style.configure("TButton", background=current_theme["accent"])

    style.map(
        "TButton", background=[("active", current_theme["accent_light"]), ("disabled", "#555555")]
    )
