...
try:
    ...
    ENGINE_HEALTH_CHECKS = prom.Counter(
        "engine_health_checks_total", "Total health checks", ["model"]
    )
    ENGINE_HEALTH_FAILURES = prom.Counter(
        "engine_health_failures_total", "Failed health checks", ["model"]
    )
                    ...
                    for cache_key, engine in engines_to_check:
                        ...
                        try:
                            ...
                            if not is_healthy:
                                ...
                                logger.warning(
                                    f"Engine {engine.model_path} health check failed "
                                ...
                                if engine.health_failures >= engine.max_failures:
                                    with cls._lock:
                                        if cache_key in cls._engines:
                                            logger.error(
                                                f"Engine {engine.model_path} failed too many health checks, "
                                            ...
                                            try:
                                                ...
                                                logger.error(
                                                    f"Error shutting down engine {engine.model_path}: {e}"
                                                )
