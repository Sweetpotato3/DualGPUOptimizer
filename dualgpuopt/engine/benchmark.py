...
DB = Path.home()/".dualgpuopt"/"benchmarks.db"; DB.parent.mkdir(exist_ok=True)
...
def _get_mid(model:str, backend:str,cfg:str)->int:
    with _lock, _conn:
        cur=_conn.execute("SELECT id FROM models WHERE model=? AND backend=? AND cfg=?",
                          (model,backend,cfg)).fetchone()
        if cur: return cur[0]
        return _conn.execute("INSERT INTO models(model,backend,cfg) VALUES(?,?,?)",
                             (model,backend,cfg)).lastrowid
...
def add(model:str, backend:str, tokps:float, **kw):
    ...
    with _lock, _conn:
        _conn.execute("INSERT INTO bench(mid,ts,tok,util,mem) VALUES(?,?,?,?,?)",
...
class BenchmarkDB:
    ...
    def _ensure_db_exists(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_benchmarks_model_id ON benchmarks(model_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_benchmarks_timestamp ON benchmarks(timestamp)")
    ...
    def add_benchmark(self, 
                     ...
                     context_size: Optional[int] = None) -> int:
        """
    ...
    def get_model_benchmarks(self, model_path: str, backend: Optional[str] = None, 
    ...
    def get_latest_benchmark(self, model_path: str, backend: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """
    ...
    def get_fastest_models(self, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for row in cursor:
                ...
                record["last_benchmark_datetime"] = datetime.fromtimestamp(timestamp).strftime("%Y-%m-%d %H:%M:%S")
                result.append(record)
                
            return result
    
    def clear_benchmarks(self, model_path: Optional[str] = None, 
...
def record_benchmark(model_path: str, 
                    backend: str,
                    tokens_per_second: float,
                    **kwargs) -> int:
    """
...
def get_model_performance(model_path: str, backend: Optional[str] = None) -> Optional[Dict[str, Any]]:
    """
