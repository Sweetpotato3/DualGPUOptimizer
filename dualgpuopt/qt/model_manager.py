...
from dualgpuopt.model.hf_client import search, download
...
class ModelManager(QWidget):
    ...
    def setup_ui(self):
        ...
        self.res.setHorizontalHeaderLabels(
            ["Model", "DLs", "Size GB", "Plan", "VRAM", "Disk", ""])
        ...
        self.local_models.setHorizontalHeaderLabels(
            ...
            for r, m in enumerate(self.models):
                ...
                if m["size"] is not None:
                    ...
                    vram_item = QTableWidgetItem(
                        f"GPU: {plan.get('gpu_layers', 'All')}" if plan.get("quant") == "gguf" 
                        else "All GPU" if not plan.get("device_map") 
                        else "Split"
                    )
                ...
                btn.clicked.connect(lambda _, idx=r: self._download(self.models[idx]["id"], fit_plan(self.models[idx]["size"] or 1<<30, gpu_query())))
    ...
    def _update_progress(self, current: int, total: int):
        ...
        self.progress_label.setText(f"Downloaded: {current/(1024*1024):.1f} MB / {total/(1024*1024):.1f} MB ({percent}%)")
    ...
    def _quantize(self):
        ...
        try:
            ...
            if method == "awq" and not model_path.suffix == ".safetensors":
                QMessageBox.warning(self, "Format Error", 
                                   "AWQ quantization requires a .safetensors model")
                return
            
            if method == "gguf" and not (model_path.suffix == ".safetensors" or 
                                         model_path.suffix == ".bin"):
                QMessageBox.warning(self, "Format Error", 
