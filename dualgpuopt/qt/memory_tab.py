        ...
        return [
            {
                ...
                "type": "SESSION_START",
                "gpu_id": -1,
                "description": "Started profiling session",
            },
            {
                "timestamp": time.time() - 40,
                "type": "INFERENCE_START",
                "gpu_id": -1,
                "description": "Started inference #1",
            },
            {
                "timestamp": time.time() - 35,
                "type": "MEMORY_ALLOCATION",
                "gpu_id": 0,
                "description": "Allocated 512MB",
            },
            {
                "timestamp": time.time() - 30,
                "type": "INFERENCE_END",
                "gpu_id": -1,
                "description": "Ended inference #1",
            },
            {
                "timestamp": time.time() - 20,
                "type": "INFERENCE_START",
                "gpu_id": -1,
                "description": "Started inference #2",
            },
            {
                "timestamp": time.time() - 15,
                "type": "MEMORY_SPIKE",
                "gpu_id": 1,
                "description": "Memory spike detected: 1024MB",
            },
            {
                "timestamp": time.time() - 10,
                "type": "INFERENCE_END",
                "gpu_id": -1,
                "description": "Ended inference #2",
            },
        ]
...
class MemoryChart(QFrame):
    ...
    def update_chart(self):
        ...
        if self.show_markers and self.markers:
            for marker in self.markers:
                ...
                if marker["time"] >= min(
                    [point[0] for gpu_data in self.filtered_data.values() for point in gpu_data],
                    default=0,
                ):
                    rel_time = marker["time"] - min(
                        [
                            point[0]
                            for gpu_data in self.filtered_data.values()
                            for point in gpu_data
                        ],
                        default=0,
                    )
    ...
    def on_mouse_move(self, event):
        ...
        # This will be used for drawing zoom rectangle in future enhancement
    ...
    def add_marker(self):
        ...
        marker_label, ok = QInputDialog.getText(
            self,
            "Add Timeline Marker",
            "Enter marker label:",
            QLineEdit.Normal,
...
class EventLogView(QFrame):
    ...
    def add_event(self, event):
        ...
        html = (
            f"<font color='#A0A0A0'>[{timestamp_str}]</font> "
        ...
        try:
            ...
            if 0 in peak_memory and 1 in peak_memory and peak_memory[0] > 0 and peak_memory[1] > 0:
                ...
                if ratio > 2 or ratio < 0.5:
                    ...
                    html = (
                        f"<p style='margin-bottom: 10px;'><span style='color: {color};'>Memory Imbalance:</span> "
            ...
            if leaks:
                for leak in leaks:
                    ...
                    html = (
                        f"<p style='margin-bottom: 10px;'><span style='color: {color};'>Memory Growth:</span> "
                ...
                for gpu_id in [0, 1]:
                    if gpu_id in peak_memory and peak_memory[gpu_id] > 0:
                        ...
                        if mb_per_inference > 5000:  # Arbitrary threshold for demonstration
                            ...
                            html = (
                                f"<p style='margin-bottom: 10px;'><span style='color: #FF9800;'>Memory Inefficiency:</span> "
            ...
            if spikes:
                ...
                for gpu_id, count in spike_count.items():
                    if count >= 3:  # Multiple spikes might indicate fragmentation
                        ...
                        html = (
                            f"<p style='margin-bottom: 10px;'><span style='color: {color};'>Possible Fragmentation:</span> "
