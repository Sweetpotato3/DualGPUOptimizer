...
def search(text:str, limit:int=30)->List[Dict[str,Any]]:
    ...
    return [{"id":m["modelId"],
             ...
             "size":next((f["size"] for f in m["siblings"]
                          if f["rfilename"].endswith((".gguf",".safetensors",".bin"))), None),
             "sha":{f["rfilename"]:f.get("sha256") for f in m["siblings"]}}
            for m in r.json()]

def download(model:str, pattern:str, dest:pathlib.Path,
             progress:Callable[[int,int],None]|None=None)->pathlib.Path:
    ...
    out = dest/f['rfilename']; part = out.with_suffix(out.suffix+".part")
    downloaded = part.stat().st_size if part.exists() else 0
    mode = "ab" if downloaded else "wb"
    hdr = dict(_HDR); hdr["Range"] = f"bytes={downloaded}-"
    with requests.get(url, headers=hdr, stream=True, timeout=60) as r, \
        ...
        for chunk in r.iter_content(64<<10):
            ...
            if progress: progress(downloaded, size)
