...
def _detect_gpus() -> List[Dict[str, int]]:
    """
    Parse `nvidia-smi` for total and free memory (MiB).
    Returns [{'id':0,'memory_total':16384,'memory_free':15000}, …]
    """
    try:
        ...
        logger.warning(f"GPU auto-detect failed: {e}")
        return []
...
def fit_plan(
    ...
    # Estimate number of model shards based on model size
    # GPT-Neox-style checkpoints often have multiple shards
    model_bytes_adjusted = model_bytes
    num_shards = 1

    # Look for checkpoint format in environment variable or estimate from size
    checkpoint_format = os.getenv("DUALGPUOPT_CHECKPOINT_FORMAT", "")
    if checkpoint_format == "gpt-neox" or checkpoint_format == "safetensors" or model_bytes > 30e9:
        # GPT-NeoX style models often have sizes that are multiples of base size
        # Estimate based on common shard counts (1, 2, 4, 8)
        if model_bytes > 80e9:
            num_shards = 8
        elif model_bytes > 40e9:
            num_shards = 4
        elif model_bytes > 20e9:
            num_shards = 2
        else:
            num_shards = 1
    ...
    hidden_size = (
        model_bytes_adjusted / 5e9
    ) * 2048  # Rough estimate of hidden size based on adjusted model size
    layers = (
        model_bytes_adjusted / 5e9
    ) * 32  # Rough estimate of layers based on adjusted model size
    ...
    plan = {
        "model_size_mb": int(model_bytes_adjusted / (1024 * 1024)),
