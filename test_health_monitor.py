...
from dualgpuopt.engine.pool import EnginePool, MAX_FAIL, CHECK_INT
...
def test_health_monitoring():
    ...
    with patch.object(Engine, '_select_backend', return_value=failing_backend):
        ...
        with patch('dualgpuopt.engine.pool._ping_backend', side_effect=lambda e: failing_backend.is_healthy()):
            ...
            logger.info(f"Monitoring engine health for {MAX_FAIL+3} health checks (check interval: {CHECK_INT}s)...")
