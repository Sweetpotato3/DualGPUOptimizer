...
class ApplicationSettingsFrame(ttk.LabelFrame):
    ...
    def __init__(
        ...
        parent: ttk.Frame,
        pad: int = 16,
        on_settings_change: Optional[Callable[[str], None]] = None,
    ) -> None:
        """
    ...
    def _create_idle_settings(self) -> None:
        ...
        idle_settings_frame.grid(
            row=0, column=0, columnspan=2, sticky="ew", padx=self.pad, pady=5
        )
        ...
        self.start_min_var = tk.BooleanVar(
            value=config_service.get("start_minimized", False)
        )
        ...
        self.idle_alerts_var = tk.BooleanVar(
            value=config_service.get("idle_alerts", True)
        )
        ...
        self.idle_threshold_var = tk.IntVar(
            value=config_service.get("idle_threshold", 30)
        )
        idle_threshold_entry = ttk.Entry(
            idle_settings_frame, textvariable=self.idle_threshold_var, width=5
        )
        ...
        idle_time_entry = ttk.Entry(
            idle_settings_frame, textvariable=self.idle_time_var, width=5
        )
