...
class OptimizerTab(ttk.Frame):
    ...
    def __init__(self, parent: ttk.Frame, gpus: List[GPU]) -> None:
        ...
        preset_names = (
            list(self.presets.keys()) if self.presets else ["No presets found"]
        )
        ...
        for i, gpu in enumerate(self.gpus):
            ...
            ttk.Label(gpu_frame, text=mem_text).grid(
        ...
        # Split string output
        ttk.Label(output_frame, text="GPU Split:").grid(
            row=0, column=0, sticky="w", padx=8, pady=8
        )
        self.split_var = tk.StringVar()
        split_entry = ttk.Entry(
            output_frame, textvariable=self.split_var, state="readonly"
        )
        split_entry.grid(row=0, column=1, sticky="ew", padx=8, pady=8)
        split_copy = ttk.Button(
            output_frame, text="Copy", command=lambda: self._copy("split")
        )
        split_copy.grid(row=0, column=2, padx=8, pady=8)

        # llama.cpp command
        ttk.Label(output_frame, text="llama.cpp:").grid(
            row=1, column=0, sticky="w", padx=8, pady=8
        )
        self.llama_var = tk.StringVar()
        llama_entry = ttk.Entry(
            output_frame, textvariable=self.llama_var, state="readonly"
        )
        llama_entry.grid(row=1, column=1, sticky="ew", padx=8, pady=8)
        llama_copy = ttk.Button(
            output_frame, text="Copy", command=lambda: self._copy("llama")
        )
        llama_copy.grid(row=1, column=2, padx=8, pady=8)

        # vLLM command
        ttk.Label(output_frame, text="vLLM:").grid(
            row=2, column=0, sticky="w", padx=8, pady=8
        )
        self.vllm_var = tk.StringVar()
        vllm_entry = ttk.Entry(
            output_frame, textvariable=self.vllm_var, state="readonly"
        )
        vllm_entry.grid(row=2, column=1, sticky="ew", padx=8, pady=8)
        vllm_copy = ttk.Button(
            output_frame, text="Copy", command=lambda: self._copy("vllm")
        )
        ...
        ttk.Label(env_frame, text="Generate Env File:").grid(
    ...
    def _preset_selected(self, *_) -> None:
        ...
        if preset_name in self.presets:
            ...
            event_bus.publish(
                "preset_selected", {"preset": preset_name, "data": preset_data}
            )
    ...
    def _load_presets(self) -> Dict[str, Any]:
        ...
        try:
            preset_path = (
                pathlib.Path(__file__).parent.parent / "presets" / "mixtral.json"
            )
