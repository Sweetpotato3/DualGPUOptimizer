...
class DualGpuApp(ttk.Frame):
    ...
    def _create_root(self, theme: str | None) -> tk.Tk:
        ...
        default_font = (
            ...
            else ("Helvetica", DEFAULT_FONT_SIZE)
        )
        ...
        try:
            icon_path = os.path.join(
                os.path.dirname(__file__), "..", "assets", "gpu_icon.ico"
            )
    ...
    def _init_services(self) -> None:
        ...
        try:
            ...
            self.log.warning(
                "GPU probe failed – switching to mock mode: %s", exc, exc_info=True
            )
    ...
    def _build_ui(self) -> None:
        ...
        self.version_label = ttk.Label(
            self.status_frame, text=f"v{VERSION}", anchor=tk.E
        )
            ...
            while True:
                ...
                if kind == "telemetry" and hasattr(self, "dashboard"):
                    ...
                    if level in ["INFO", "WARNING", "ERROR"]:
                        self.status_var.set(
                            message[:160] + ("..." if len(message) > 160 else "")
                        )
    ...
    def _handle_command_error(self, error_data: Dict[str, Any]) -> None:
        ...
        # Log the error
        self.log.error(
            f"Command error: {error_data.get('message', 'Unknown command error')}"
        )
