...
from dualgpuopt.engine.benchmark import get_fastest_models, benchmark_db
...
class TokenGeneratorBackend:
    ...
    def __init__(self, tokens_per_second: float = 20.0, gpu_util: float = 60.0,
                memory_usage: float = 4000.0):
...
def simulate_chat_session(model_path: str, backend: TokenGeneratorBackend, 
                         num_messages: int = 3):
    """
    ...
    with patch.object(Engine, '_select_backend', return_value=backend):
        ...
        for i in range(num_messages):
            ...
            logger.info(f"Generated {token_count} tokens in {elapsed:.2f}s ({tokens_per_second:.2f} tokens/s)")
...
def test_benchmark_system():
    ...
    logger.info(f"Total benchmarks: {len(benchmark_db.get_model_benchmarks('/models/fast-70b-v2'))}")
